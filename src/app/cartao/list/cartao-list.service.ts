...
import cartaoExporterFields from 'src/app/cartao/list/cartao-exporter-fields';
...
export class CartaoListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await CartaoApi.list(
        filter,
        this.orderBy,
        null,
        null,
