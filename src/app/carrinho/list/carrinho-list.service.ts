...
import carrinhoExporterFields from 'src/app/carrinho/list/carrinho-exporter-fields';
...
export class CarrinhoListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await CarrinhoApi.list(
        filter,
        this.orderBy,
        null,
        null,
