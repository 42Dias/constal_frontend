...
import pessoaFisicaExporterFields from 'src/app/pessoa-fisica/list/pessoa-fisica-exporter-fields';
...
export class PessoaFisicaListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await PessoaFisicaApi.list(
        filter,
        this.orderBy,
        null,
        null,
